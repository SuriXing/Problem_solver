...
if command -v python3 &> /dev/null; then
    ...
    echo -e "${RED}Error: Neither python nor python3 command was found. Please install Python.${NC}"
    exit 1
fi

# Check if the worry-solver directory exists
WORRY_SOLVER_PATH="../worry-solver"
if [ ! -d "$WORRY_SOLVER_PATH" ]; then
    WORRY_SOLVER_PATH="../../worry-solver"
    if [ ! -d "$WORRY_SOLVER_PATH" ]; then
        echo -e "${RED}Error: Could not find the worry-solver directory.${NC}"
        echo "Please provide the path to the worry-solver directory:"
        read -p "> " WORRY_SOLVER_PATH
        
        if [ ! -d "$WORRY_SOLVER_PATH" ]; then
            echo -e "${RED}Error: The provided path is not a valid directory.${NC}"
            exit 1
        fi
    fi
fi
...
start_original() {
  ...
  pkill -f "$PYTHON_CMD -m http.server 3000" || true
  
  # Get parent directory of current directory (assuming we're in visual-comparison)
  cd ..
  PROBLEM_SOLVER_DIR=$(pwd)
  echo "Starting server in $PROBLEM_SOLVER_DIR"
...
start_react() {
  ...
  # Check if the directory exists and cd into it
  if [ -d "$WORRY_SOLVER_PATH" ]; then
    echo "Worry solver directory found at: $WORRY_SOLVER_PATH"
    cd "$WORRY_SOLVER_PATH"
    
    # Check if package.json exists
    if [ ! -f "package.json" ]; then
      echo -e "${RED}Error: package.json not found in $WORRY_SOLVER_PATH. Not a React application.${NC}"
      cd - > /dev/null
      return 1
    fi
    ...
    echo "Access at http://localhost:3000"  # React's default port is 3000
    
    # Return to the original directory
    cd - > /dev/null
  else
    echo -e "${RED}Error: worry-solver directory not found.${NC}"
    return 1
  fi
}
...
capture_screenshots() {
  ...
  if ! npm list -g puppeteer &> /dev/null; then
    ...
    if [ $? -ne 0 ]; then
      echo -e "${RED}Error: Failed to install puppeteer. Please try manually: npm install -g puppeteer${NC}"
      return 1
    fi
  fi
  
  # Determine the React app port (assume 3000 unless specified otherwise)
  REACT_PORT=3000
...
const pages = [
  { name: 'home', original: 'http://localhost:3000/index.html', react: 'http://localhost:${REACT_PORT}/' },
  { name: 'help', original: 'http://localhost:3000/help.html', react: 'http://localhost:${REACT_PORT}/help' },
  { name: 'help-detail', original: 'http://localhost:3000/help-detail.html', react: 'http://localhost:${REACT_PORT}/help/detail/1' },
  { name: 'help-success', original: 'http://localhost:3000/help-success.html', react: 'http://localhost:${REACT_PORT}/help/success' },
  { name: 'past-questions', original: 'http://localhost:3000/past-questions.html', react: 'http://localhost:${REACT_PORT}/past-questions' },
  { name: 'submit-query', original: 'http://localhost:3000/submit-query.html', react: 'http://localhost:${REACT_PORT}/submit' },
  { name: 'success', original: 'http://localhost:3000/success.html', react: 'http://localhost:${REACT_PORT}/success' },
  { name: 'topic-detail', original: 'http://localhost:3000/topic-detail.html', react: 'http://localhost:${REACT_PORT}/topic/1' },
  { name: 'share', original: 'http://localhost:3000/share.html', react: 'http://localhost:${REACT_PORT}/share' }
];
...
EOF
  ...
  if [ $? -ne 0 ]; then
    echo -e "${RED}Error: Failed to capture screenshots. See error messages above.${NC}"
  else
  ...
  <script>
    ...
    pages.forEach(page => {
      ...
      viewports.forEach((viewport, index) => {
        ...
        originalContainer.innerHTML = `
          ...
          <div class="image-wrapper" id="original-${page.name}-${viewport.name}"></div>
        `;
        ...
        reactContainer.innerHTML = `
          ...
          <div class="image-wrapper" id="react-${page.name}-${viewport.name}"></div>
        `;
    ...
    pages.forEach(page => {
      viewports.forEach(viewport => {
        const originalPath = `screenshots/problem_solver/${page.name}_${viewport.name}.png`;
        const reactPath = `screenshots/worry_solver/${page.name}_${viewport.name}.png`;
        
        // Check original image
        checkImage(originalPath, function(exists) {
          const container = document.getElementById(`original-${page.name}-${viewport.name}`);
          if (exists) {
            container.innerHTML = `<img src="${originalPath}" alt="Original ${page.title} (${viewport.name})">`;
          } else {
            container.innerHTML = `<div class="missing-image">Image not available</div>`;
          }
        });
        
        // Check React image
        checkImage(reactPath, function(exists) {
          const container = document.getElementById(`react-${page.name}-${viewport.name}`);
          if (exists) {
            container.innerHTML = `<img src="${reactPath}" alt="React ${page.title} (${viewport.name})">`;
          } else {
            container.innerHTML = `<div class="missing-image">Image not available</div>`;
          }
        });
